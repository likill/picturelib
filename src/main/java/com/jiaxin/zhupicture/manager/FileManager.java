package com.jiaxin.zhupicture.manager;
...
import com.jiaxin.zhupicture.config.CosClientConfig;
...
public class FileManager {
    ...
    private void validPicture(String fileUrl){
        //1、文件不能为空
        ThrowUtils.throwIf(StrUtil.isBlank(fileUrl), ErrorCode.PARAMS_ERROR, "文件路劲不能为空");
        //2、校验url格式
        try {
            new URL(fileUrl);
        ...
        //3、校验url协议
        ThrowUtils.throwIf(!fileUrl.startsWith("http://")&&!fileUrl.startsWith("https://"), ErrorCode.PARAMS_ERROR, "仅支持http://或https://文件地址");
        //4、发送head请求验证文件是否存在
        ...
        try{
            ...
            if(StrUtil.isNotBlank(contentType)){
                //允许图片类型
                final List<String> ALLOW_FORMAT_LIST = Arrays.asList("image/jpeg", "image/jpg", "image/png", "image/gif");
